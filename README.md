# Laravel NuxtJS Typescript Boilerplate

A base template for `NuxtJS (^3.8.2)` with backend API implementation using `Laravel (^11.0)` preconfigured `laravel/passport(^12.0)` authentication.
...
```
APP_DOMAIN=nuxt-base.local                # for local development
```

in this case: https://nuxt-base.local/api/v1
...
This package uses NuxtJS as frontend framework. This docker setup will automatically serve the NuxtJS via node container.
...
```
NUXT_ENV_SITE_TITLE="My Site/App Title"  // The Suffix Site Title for all Pages  e.g Page - NUXT_ENV_SITE_TITLE
NUXT_ENV_CLIENT_ID=                      // GENERATED FROM php artisan passport:install
NUXT_ENV_CLIENT_SECRET=                  // GENERATED FROM php artisan passport:install
NUXT_ENV_API_URL=                        // THE API DOMAIN SET ON DOCKER ENV FILE
NUXT_ENV_SSR_URL=http://nginx/api/v1     // For SSR requests, don't change
NUXT_ENV_DEFAULT_LOCALE=en               // Valid values "en" or "ja"
```
...
npm run lint         # To check for possible production build error
npm run format       # To format code automatically to follow the standards
```
...
The latest **NuxtJS** version has disabled automatic linting and Typescript checking on file save. To enable Typescript checking on file save, you will need to run the following command.  
```
docker compose exec -it PROJECT_node sh
npm run ts:watch
```
...
4. Run the NuxtJS Build Manually
